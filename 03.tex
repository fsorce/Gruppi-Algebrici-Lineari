...
Se $G$ e $H$ sono gruppi algebrici affini con $\vp:G\to H$ omomorfismo di gruppi algebrici allora $\vp(G)$ \`e un chiuso di $H$, e quindi a sua volta un gruppo algebrico affine.
\end{proposition}
\begin{proof}
Sia $T=\vp(G)\subseteq H$. Vogliamo mostrare che $T=\ol T\subseteq H$.  Notiamo che $\ol T$ \`e un sottogruppo di $H$:
\setlength{\leftmargini}{0cm}
\begin{itemize}
\item[$\boxed{i(\ol T)=\ol T}$] Osserviamo che $i(T)\subseteq T$ in quanto sottogruppo. Per continuit\`a di $i$ abbiamo dunque che $i\ii(\ol T)\supseteq \ol T$, e quindi $i(\ol T)\subseteq \ol T$ come voluto.
\item[$\boxed{\mu(\ol T\times \ol T)=\ol T}$] Per ogni $t\in T$ sappiamo che $\mu(t,T)\subseteq T$, dunque $t\ii\cdot \ol T=\mu(t,\cdot)\ii(\ol T)\supseteq \ol T$, ovvero $t\ol T\subseteq \ol T$ per ogni $t\in T$. Se ora prendiamo $t'\in \ol T$ questo contenimento mostra in particolare che $t't\in \ol T$ per ogni $t\in T$, cio\`e $\mu(t',T)\subseteq \ol T$, da cui $(t')\ii\cdot \ol T\supseteq \ol T$ e quindi $t'\ol T\subseteq \ol T$ per ogni $t'\in \ol T$, cio\`e $\mu(\ol T\times \ol T)\subseteq \ol T$.
\end{itemize}
\setlength{\leftmargini}{0.5cm}

Per il teorema (\ref{ThDominantiAperteSuAperto}), $T\supseteq U$ per $U$ aperto di $\ol T$. Dunque per ogni $t\in \ol T$, $tU\subseteq \ol T$.
...
%Possiamo adesso dimostrare un importante risultato sugli omomorfismi di gruppi algebrici
...
%    Per il teorema (\ref{ThDominantiAperteSuAperto}) esiste un aperto $U$ di $X$ contenuto in $\varphi(G)$. Poiché $X$ e $\varphi(G)$ sono sottogruppi di $H$, si ha \[U\cdot U \subseteq \varphi(G)\subseteq X.\]
...
\section{Rappresentazioni regolari}
...
\begin{lemma}\label{LmRepresentationActsLinearly}
...
\end{proof}
\begin{remark}
Il lemma semplicemente afferma che per ogni $g\in G$, se $h\in V^\ast$ allora $v\mapsto h(gv)$ \`e ancora un funzionale su $V$.
\end{remark}


\begin{lemma}\label{LmAnelloCoordinateERappresentazioneRegolare}
...
\setlength{\leftmargini}{0cm}
\begin{itemize}
\item[$\boxed{\dim \ps{Gf}<\infty}$] Sia $f\in \K[X]$. Ponendo $\sigma^\ast(f)=\sum_{i=1}^k \al_i\otimes \beta_i$ con somma finita, si ha
\[f(gx)=f\circ\sigma(g,x)=\sigma^\ast(f)(g,x)=\sum_i \al_i(g)\beta_i(x),\]
quindi
...
\item[$\boxed{\text{reg. su fin.dim.}}$] Vogliamo dimostrare che su $W\subseteq \K[X]$ di dimensione finita l'azione di $G$ \`e regolare, cio\`e che $\sigma^\ast\res{\K[W]}$ \`e un omomorfismo di $\K$-algebre con codominio $\K[G]\otimes \K[W]=\K[G\times W]$. Notiamo che basta verificare che $\sigma^\ast\vp$ \`e regolare per una classe di $\vp$ che generano $\K[W]$, per esempio $\vp\in W^\ast$. Vogliamo dunque mostrare che $\psi=\sigma^\ast\vp$ data da $\psi(g,f)=\vp(gf)$ \`e regolare su $G\times W$ per ogni $\vp\in W^\ast$.

Sia $f_1,\cdots, f_k$ una base di $W$ e $f_1^\ast,\cdots,f_k^\ast$ la relativa base duale di $W^\ast$. Poich\'e sono una base e $W$ \`e $G$-stabile, $\sigma^\ast(f_i)=\sum_j\al_{ij}\otimes f_j$. Ponendo $\vp(f_i)=\la_i$ si ha
\begin{align*}
\psi(g,f)=&\vp\pa{g\cdot\sum_{i=1}^k f_i^\ast(f)f_i}=\vp\pa{\sum_{i=1}^k \sum_{j=1}^k f_i^\ast(f)\al_{ij}(g\ii)f_j}=\\
=&\sum_{j}\pa{\sum_i\al_{ij}(i(g))f_i^\ast(f)}\la_j,
\end{align*}
che \`e regolare su $G\times W$ in quanto $i:G\to G$ \`e l'inversa (morfismo) e $f_i^\ast\in W^\ast\subseteq \K[W]$.
\end{itemize}
\setlength{\leftmargini}{0.5cm}
\end{proof}
...
\end{theorem}
\begin{proof}[Dimostrazione (per Esercizio)]
Scriviamo $\K[X]=\frac{\K[x_1,\cdots, x_n]}{I}$ per qualche ideale $I$. Chiamiamo $f_1,\cdots, f_n$ le classi di $x_1,\cdots, x_n$ in $\K[X]$. Poich\'e $\K[X]$ \`e una rappresentazione regolare di $G$ per (\ref{LmAnelloCoordinateERappresentazioneRegolare}), possiamo considerare $H\subseteq \K[X]$ sottorappresentazione di dimensione finita che contiene $f_1,\cdots, f_n$. Sia $V=H^\ast$ lo spazio vettoriale duale (che ha anello delle coordinate $SH$).

La mappa di algebre $SH\to \K[X]$, che \`e evidentemente $G$-equivariante, corrisponde al morfismo $X\to V$ che ad un punto $x$ associa il funzionale $f\mapsto f(x)$ per $f\in H$. Questo morfismo \`e equivariante per la proposizione precedente ed \`e iniettivo perch\'e se $x$ e $y$ hanno la stessa immagine allora $f_i(x)=f_i(y)$ per ogni $i$, ma $f_i$ \`e la classe di $x_i$ in $\K[X]$, quindi questo significa che $x$ e $y$ sono lo stesso punto di $X\subseteq \K^n$.
\end{proof}
