...
Se $g\in G$ allora $g$ \`e semisemplice/unipotente/nilpotente se e solo se l'azione di $g$ su $\K[G]$ \`e semisemplice/unipotente/nilpotente.
\end{corollary}
\begin{proof}
Facciamo il caso semisemplice, gli altri sono identici.
\setlength{\leftmargini}{0cm}
\begin{itemize}
...
\begin{lemma}[Criterio per semisemplice/unipotente in gruppi lineari]\label{LmCriterioSemisempliceUnipotentePerGruppiLineari} Sia $V$ uno spazio vettoriale di dimensione finita e sia $G\subseteq GL(V)$ un sottogruppo chiuso. Allora $g\in G$ \`e semisemplice/unipotente se e solo se l'azione di $g$ su $V$ \`e semisemplice/unipotente.
\end{lemma}
\begin{proof}
Diamo le implicazioni per il caso semisemplice. Il caso unipotente \`e analogo.
\setlength{\leftmargini}{0cm}
\begin{itemize}
\item[$\boxed{\implies}$] Ovvio
...
Se $\dim V=n$ allora $\End(V)$ con l'azione di moltiplicazione a sinistra di $g$ \`e uguale a considerare l'azione di $g$ su $V^{\oplus n}$ dove la corrispondenza \`e data dal fatto che l'azione per moltiplicazione a sinistra agisce sulle colonne della matrice per restituire le colonne della matrice risultato, cio\`e
\[g\pa{v_1\mid \cdots\mid v_n}=\pa{gv_1\mid \cdots\mid gv_n}.\]
...
Consideriamo allora l'azione di $g$ su $\K[\End(V)]\otimes \K$ dove sulla copia di $\K$ abbiamo\footnote{questo punto \`e dove la dimostrazione fallirebbe se volessimo anche il caso di $g$ nilpotente.} $g\la=(\det g)^{-N}\la$. Abbiamo una mappa $G$-equivariante surgettiva
\[\funcDef{\K[\End(V)]\otimes \K}{\frac1{{\det}^N}\K[\End(V)]}{f\otimes \la}{\frac\la{(\det)^N}f}\]
quindi, poich\'e $g$ agisce in modo semisemplice su $\K[\End(V)]$ e su\footnote{nella versione della dimostrazione per $g\in \GL(V)$ unipotente avremmo che $\det g=1$ e quindi effettivamente $g$ agirebbe in modo unipotente su $\K$.} $\K$, si ha che agisce in modo semisemplice su $\frac1{{\det}^N}\K[\End(V)]$ e quindi su $W$.
...
\begin{proposition}\label{PrDecomposizioneSemisempliceNilpotente}
    ...
    \begin{enumerate}
        ...
        \item[4.] Se $V'$ è un $\K$-spazio vettoriale di dimensione finita, $T'$ è un endomorfismo di $V'$ e $L\colon V \to V'$ è un'applicazione lineare tale che $L\circ T=T'\circ L$, allora \[L\circ T_s = T'_s\circ L \quad \text{ e } \quad L\circ T_n = T'_n\circ L.\]
    \end{enumerate}
\end{proposition}

\begin{proof}
Mostriamo prima il punto 2. e poi gli altri.
\setlength{\leftmargini}{0cm}
\begin{enumerate}
    \item[2.] Fissando una base di $V$ dove $T$ \`e in forma di Jordan, sia $S$ l'endomorfismo di $V$ dato dalla diagonale di $T$ in questa base e $N=T-S$.
    
    Consideriamo il polinomio caratteristico $p_T$ di $T$ e scriviamolo nella forma \[p_T(t)=\prod_{i=1}^r(t-\lambda_i)^{n_i},\] dove i $\lambda_i$ sono distinti. Cerchiamo un polinomio $f(t)$ in $\K[t]$ tale che\footnote{la congruenza $f(t)\equiv 0\pmod{(t)}$ serve a garantire che $f(0)=0$.} 
    \[\begin{cases}
    f(t)\equiv 0 &\pmod{(t)}\\
    ...
    \item[1.] Siano $S$ e $N$ definiti a partire dalla forma di Jordan di $T$ come sopra. Evidentemente $S$ \`e semisemplice, $N$ \`e nilpotente e $SN=NS$.
    
    Mostriamo l'unicità: se $S'$ e $N'$ sono tali che $T=S+N=S'+N'$ e $S'N'=N'S'$, allora $S$ commuta con $S'$ perch\'e $S'$ commuta con $T$ ($S'T=S'S'N=S'NS'=TS'$) e $S=f(T)$ per $f$ polinomio. Segue dunque che $S$ e $S'$ sono semisemplici che commutano, dunque sono simultaneamente diagonalizzabili. Fissando una base che realizza questa simultanea digonalizzabilit\`a, osserviamo la matrice associata a $S-S'=N'-N$ in questa base \`e diagonale (LHS) e nilpotente (RHS), dunque la matrice \`e nulla e troviamo $S=S',\ N=N'$ come voluto. 
    \item[3.] Poiché $S=f(T)$ e $N=g(T)$, se $W$ è $T$-stabile allora \`e chiaramente anche $T_s$- e $T_n$-stabile. Siano ora $t=\left.T\right|_{W}$, $s=\left.T_s\right|_{W}$ e $n=\left.T_n\right|_{W}$. Allora $t=s+n$, $sn=ns$ e, per il lemma (\ref{LmPassaggioSemisempliceUnipotenteNilpotenteAdOperazioniVettoriali}), $s$ è semisemplice e $n$ è nilpotente. Quindi la tesi discende dall'unicità della decomposizione.
    \item[4.] Consideriamo il diagramma:
...
\end{tikzcd}\]    dove $L_1 \colon v \mapsto (v,L(v))$ e $L_2\colon (v,w)\mapsto w$. Osserviamo che in una base di $V\oplus V'$ che rispetta questa decomposizione si ha che $T''=\pa{\smat{T &0\\0&T'}}$, dunque 
\[T''_s=\mat{T_s &0\\0&T'_s}=T_s\oplus T'_s\] 
perch\'e $\pa{\smat{T_s &0\\0&T'_s}}$ \`e diagonalizzabile (afficando una base di $V$ che diagonalizza $T_s$ e una di $V'$ che diagonalizza $T'_s$) e $T''-\pa{\smat{T_s &0\\0&T'_s}}=\pa{\smat{T_n &0\\0&T'_n}}$ \`e evidentemente nilpotente.


Da questo segue che $L_1\circ T_s=T''_s\circ L_1$ e $L_2\circ T''_s=T'_s\circ L_2$. Deduciamo quindi che \[L\circ T_s =L_2\circ L_1 \circ T_s= L_2 \circ T''_s\circ L_1= T'_s\circ L_2 \circ L_1= T'_s\circ L.\]
\end{enumerate}
\setlength{\leftmargini}{0.5cm}
...
Mostriamo ora l'unicit\`a. Supponiamo di avere $S',U'$ con $T=S'U'=N'U'$. Sia $v\in V$. Essendo $T$ localmente finita esiste un sottospazio $T$-stabile di dimensione finita $W$ che contiente $v$. Evidentemente $T\res W=S'\res W U'\res W$, ma sappiamo che in dimensione finita vale l'unicit\`a, dunque $S'\res W=S\res W$ e quindi in particolare $S'(v)=S(v)$. Analogamente $U'(v)=U(v)$.

Osserviamo che $S$ e $U$ costruite soddisfano le proprietà\footnote{ma NON la 2.} 3 e 4 dell'enunciato precedente. 
...
\[U(n)=\cpa{\mat{1&\ast&\ast\\&\ddots&\ast\\&&1}}.\]
\end{notation}

\begin{remark}
I sottogruppi chiusi di $U(n)$ sono chiaramente unipotenti per il lemma (\ref{LmCriterioSemisempliceUnipotentePerGruppiLineari}). In realt\`a mostreremo tra poco che tutti i gruppi unipotenti sono di questa forma.
\end{remark}


\begin{theorem}[Caratterizzazione dei gruppi unipotenti]\label{ThCaratterizzazioneGruppiUnipotenti}
Sia $G$ un gruppo algebrico affine, i seguenti fatti sono equivalenti
\begin{enumerate}
\item $G$ \`e unipotente
\item Le sole rappresentazioni irriducibili di $G$ sono banali
\item $G$ si immerge nel gruppo $U(n)$.
\end{enumerate}
\end{theorem}
\begin{proof}
...
\setlength{\leftmargini}{0cm}
\begin{itemize}
\item[$\boxed{1.\implies2.}$] Sia $V$ una rappresentazione semplice di $G$ di dimensione $n$. Allora\footnote{tiriamo in gioco la traccia perch\'e svolger\`a il ruolo di un prodotto scalare definito positivo su $\End_\K(V)$, che mostriamo essere $\Span(G)$ grazie al teorema.} $\tr(g_V)=n$ per ogni $g\in G$ (perch\'e si immerge nelle triangolari superiori), quindi
\[\forall g_V,h_V\in G,\qquad \tr((g_V-1)h_V)=\tr(g_Vh_V-h_V)=0.\]
Se $A$ \`e il $\K$-spazio vettoriale generato da $G$, $A\subseteq \End_\K(V)$ ed \`e un'algebra associativa. Poich\'e $V$ \`e semplice per $A$ (in quanto semplice per $G$ e $A=\Span_\K(G)$), si ha per un corollario del lemma di Schur (\ref{ThSottoalgebraAssociativaDiEndomorfismiDiSempliceETuttiGliEndomorfismi}) che $A=\End_\K(V)$.

Segue per linearit\`a della traccia che $\tr((g_V-1)a)=0$ per ogni $a\in \End_\K(V)=A$, da cui $(g_v-1)=0$, cio\`e $G$ agisce banalmente.
\item[$\boxed{2.\implies3.}$] Sia $V$ un $\K$-spazio vettoriale di dimensione finita per cui $G$ è un sottogruppo di $\GL(V)$. Sia $W$ una sottorappresentazione di $V$ irriducibile non nulla. Allora $W=\K v_1$ e $g v_1=v_1$ per ogni $g$ in $G$. Consideriamo il quoziente $V_1=V/\langle v_1\rangle$. Se $V_1=0$ abbiamo la tesi, altrimenti $V_1$ \`e una rappresentazione di $G$ non nulla e perci\`o ammette una sottorappresentazione banale irriducibile non nulla $\K(v_2+\ps{v_1})\subseteq V_1$. In particolare $v_2\in V$ \`e tale che per ogni $g$ in $G$ si ha $gv_2\equiv v_2 \pmod{\K v_1}$. Procedendo in questo modo, possiamo scegliere una base $v_1,\ldots,v_n$ di $V$, rispetto alla quale risulta per costruzione $G\subseteq U(n)$.
\item[$\boxed{3.\implies1.}$] Segue immediatamente dal lemma (\ref{LmCriterioSemisempliceUnipotentePerGruppiLineari}).
\end{itemize}
\setlength{\leftmargini}{0.5cm}
\end{proof}
...
\end{proposition}
\begin{proof}
Dato i gruppi algebrici affini sono lineari possiamo immergere $G\subseteq \GL(V)$. Scegliamo una base di $V$ in modo tale che la matrice associata a $g$ appartenga a $U(n)$. Dato che $g\neq id_G$, $\log g=x\neq 0$ in $N(n)$, inoltre $\log(g^n)=nx$ in quanto $x$ commuta con se stesso e quindi $exp(nx)=(\exp x)^n=g^n$. 
%Notiamo che da $nx\neq 0$ per ogni $n$ ricaviamo $g^n\neq id_G$ per ogni $n$. 
Se $Y=\K x\subseteq N(n)$ allora
\[\exp(Y)=\log\ii(Y)\supseteq \cpa{g^n}\implies \ol{\cpa{g^n}}^{U(n)}\subseteq\exp(Y).\]
Notiamo che $G$ e $U(n)$ sono chiusi di $\GL(n)$ che contengono $\cpa{g^n}$, quindi $\ol{\cpa{g^n}}^{U(n)}=\ol{\cpa{g^n}}^G=H$.

Abbiamo dunque mostrato che $H\subseteq \exp(Y)$.
Poich\'e $Y$ \`e una retta e $\cpa{nx}$ sono infiniti, $\ol{\cpa{nx}}= Y$ per come sono fatti i chiusi di Zariski di $\A^1$, quindi quel contenimento \`e una uguaglianza e
\[\funcDef{\K}{H=\exp(Y)}{\la}{\exp(\la x)}\]
\`e un isomorfismo di gruppi tra $(\K,+)$ e $H\subseteq G$.
\end{proof}
...
Sia $V$ una rappresentazione irriducibile di $G$ e sia $U\subseteq G$ un sottogruppo normale unipotente. Poich\'e $V\neq (0)$ e $U$ unipotente, $V^U\neq 0$ per il corollario (\ref{CorFissatoDaUnipotenteNonEBanale}). Poich\'e $U$ \`e normale, $V^U$ \`e stabile per $G$ e quindi\footnote{$V^U$ \`e una sottorappresentazione di $V$ e $V$ \`e irriducibile, dunque $V^U=V$ oppure $V^U=0$, ma abbiamo appena mostrato che $V^U\neq 0$.} $V^U=V$, cio\`e $U$ agisce banalmente su tutte le rappresentazioni irriducibili. Questo mostra che per la mappa $G\to \GL(W)$ si ha che $U$ finisce in $\cpa{id_W}$ perch\'e $G$ \`e completamente riducibile, ma questa mappa \`e iniettiva e quindi $U=\cpa{id_G}$.
\end{proof}
...
\begin{remark}
Le rappresentazioni semplici di $G$ sono di dimensione 1 e sono date da $\K_\al=\K t^{-\al}$ con $gz=t^\al(g)z$.
\end{remark}
